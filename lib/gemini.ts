...
export async function generateDynamicContent(
  ...
  category: string
): Promise<{ narration: string; youtubeDescription: string; title?: string; titleB?: string; hookTitle?: string }> {
...
以下をJSON形式で返してください:
{
  ...
  "narration": "【書き出しは必ず『実は〜』『〜って知ってました？』『〜が変わる理由があります』のいずれかで始める】共感・具体性・フックを強化した30秒以内の自然な日本語ナレーション。エンゲージメント誘導（いいね・チャンネル登録・コメント）を末尾に含める。100文字以内。",
  "youtubeDescription": "YouTube説明欄用テキスト3〜4文。トピックの背景・具体的なtips・視聴者へのメッセージ。CTAを最後に含める。200文字以内。"
}
JSONのみ返してください。`;
  ...
  try {
    ...
    const parsed = JSON.parse(text) as { narration: string; youtubeDescription: string; titleA?: string; titleB?: string; hookTitle?: string };
    if (parsed.narration && parsed.youtubeDescription) {
      return {
        ...
        hookTitle: parsed.hookTitle,  // 疑問文フック（スライド1用）
      };
    }
  } catch (e) {
