...
export function inferWeight(topic: string): number {
  ...
  // ─── 重み6: API実測いいね率18倍 NG暴露・比較型（2026-05-12 プロデューサー更新）───
  // 「〇〇より△△の理由」逆説・比較型（資格より実績 = 1,603再生 / いいね率最高）
  if (/より.*理由|より.*効く|より.*有利|より.*大切|より.*重要|より.*強い/.test(t)) return 6;
  // NG行動暴露型（転職面接NG行動 = 469再生 / いいね率0.43%最高）
  if (/NG行動|やってはいけない|してはいけない|やりがちなミス|ありがちな失敗|落とし穴/.test(t)) return 6;
  // 「言わない真実・知らない事実」暴露型
  if (/言わない|知らない|教えない|隠れた|本当の理由|実は|意外と/.test(t)) return 6;
  ...
  // ループ誘発型（AIで知識定着 = 110%視聴率）
  if (/理由を知っていますか|知っていましたか|気づいていない|実は.*できる|こんなに違う/.test(t)) return 5;
